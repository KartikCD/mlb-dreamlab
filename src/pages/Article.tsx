...
const renderContentBlock = (block: ContentBlock, index: number) => {
  switch (block.type) {
    case 'paragraph':
      return <p key={index} className="text-lg md:text-xl text-text-secondary leading-relaxed mb-6">{block.text}</p>;
    case 'image':
      return (
  ...
  if (!article) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center text-center px-6">
        ...
        <Link to="/news" className="text-champagne-gold hover:underline flex items-center">
      ...
      <div className="relative -mt-48 md:-mt-64 pb-20">
        <div className="container mx-auto px-6 max-w-4xl">
          ...
          >
            <div className="bg-steel-gray-darker p-8 md:p-12 rounded-lg shadow-2xl card-glow-subtle">
              <div className="flex items-center text-sm text-text-secondary mb-4 space-x-6">
                ...
                <div className="flex items-center">
                  <Calendar className="w-4 h-4 mr-2 text-champagne-gold" />
                  <span>{new Date(article.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                </div>
                <div className="flex items-center">
                  <User className="w-4 h-4 mr-2 text-champagne-gold" />
                  <span>{article.author}</span>
                </div>
              </div>
              <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tighter text-glow mb-8">{article.title}</h1>
              <p className="text-xl md:text-2xl text-text-secondary font-light italic">{article.excerpt}</p>
            </div>
          </motion.div>
          ...
          <div className="mt-16 text-center">
            <Link to="/news" className="inline-flex items-center text-champagne-gold hover:text-white transition-colors duration-300 text-lg">
              <ArrowLeft className="w-5 h-5 mr-2" />
              Return to News
            </Link>
          </div>
